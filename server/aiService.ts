...
export async function generateAIResponse(userMessage: string, context?: any): Promise<string> {
  try {
    ...
    if (isFullMode) {
      ...
      // If it's not a specific command but a general request, create a task for it
      const isActionRequest = userMessage.match(/\b(create|build|make|fix|update|add|remove|delete|improve|optimize|analyze)\b/i);
      if (isActionRequest && !executionResult.success) {
        try {
          // Create a task for this action and assign it to the actual user
          const task = await storage.createTask({
            title: `🤖 ${userMessage.slice(0, 50)}${userMessage.length > 50 ? '...' : ''}`,
            description: `[AI Task] ${userMessage}`,
            projectId: context?.projectId || 'default-project',
            assigneeId: context?.userId || 'default-user', // Assign to real user, not fake people!
            status: 'in_progress',
            priority: 'medium',
          ...
          console.log(`✅ Created task for user request: ${task.title}`);
          ...
          return `Working on: ${userMessage}\n\nI've created a task and I'm executing it now. You'll see it in your Tasks view.`;
        } catch (error) {
